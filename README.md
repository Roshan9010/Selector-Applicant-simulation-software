...
This project includes a small migration helper `backend/migrate_add_columns.py` that will run automatically when the backend starts. It safely adds any missing columns (used by the current models) to the SQLite database without dropping data. If you prefer to run it manually:
...
To manage migrations with Alembic, a lightweight helper `backend/manage_migrations.py` is included. It wraps common Alembic commands so you can run them from the project root. Examples:

```bash
# Show current DB revision
python backend/manage_migrations.py current

# Apply all migrations
python backend/manage_migrations.py upgrade head

# Create a new autogenerate revision (remember to provide a message)
python backend/manage_migrations.py revision --autogenerate -m "add new field"

# Rollback one revision
python backend/manage_migrations.py downgrade -1
```

Note: Ensure your Python virtual environment is active so `alembic` is available (it is listed in `backend/requirements.txt`).

#### SMTP / Email configuration

To enable automatic sending of interview invitations you must configure SMTP environment variables for the backend service. Example environment variables:
...
SMTP_FROM=Recruiter <recruiter@example.com>   # optional; defaults to SMTP_USER
```

When configured, pressing "Send Interview Invitation" in the admin UI will create the invitation and email the candidate automatically. If SMTP is not configured the backend will still create invitations but will return `email_sent: false` and an `email_error` message in the response.

#### Optional: AI-composed email content

If you want the system to compose polished invitation emails automatically, set an OpenAI API key in the backend environment:
...
When `OPENAI_API_KEY` is provided the backend will ask the model to generate a subject, plain-text body and an HTML body for the invitation. If the key is not present, the backend falls back to a simple template.

#### Local development: email logging fallback

If SMTP is not configured, the backend will not fail — instead it logs the composed email to the console and appends it to `backend/email_log.txt`. This makes it easy to test invitation flows locally without sending real emails.

Example log entry is appended to `backend/email_log.txt` when an invitation is created.
